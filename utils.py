...
class MyPriorityQueue(PriorityQueue):
    def __init__(self, order=min, f=lambda x: x):
        self.order = order
        self.f = f
        self.counter = 0  # Used to break ties in a stable way
        super().__init__()

    def put(self, item, priority=None, order=None, count=None):
        super().put((self.f(item), order, count, item), priority)

    def get(self):
        _, _, _, item = super().get()
        return item

    def __contains__(self, item):
        return some(lambda __x: __x[3] == item, self.queue)

    def __getitem__(self, key):
        for _, _, _, item in self.queue:
            if item == key:
                return item

    def __delitem__(self, key):
        for i, (_, _, _, item) in enumerate(self.queue):
            if item == key:
                self.queue.pop(i)
                return

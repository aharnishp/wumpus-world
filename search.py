...
class Node:
    ...
    def __lt__(self, other):
        """Override the less-than operator to enable comparisons in PriorityQueue."""
...
def best_first_graph_search(problem, f):
    ...
    while not frontier.empty():
        ...
        explored.add(node.state)
        for child in node.expand(problem):
            if child.state not in explored and not any(item[1] == child for item in frontier.queue):
                frontier.put((f(child), child))
            elif any(item[1] == child for item in frontier.queue):
                incumbent = next(item[1] for item in frontier.queue if item[1] == child)
                if f(child) < f(incumbent):
                    frontier.queue.remove((f(incumbent), incumbent))
                    frontier.put((f(child), child))
